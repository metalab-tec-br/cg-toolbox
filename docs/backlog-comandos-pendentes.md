...
- **Diretriz confirmada (2026-07-21):** incluir TODAS as variações de cada comando (consulta/get/show, alteração/set, exclusão/del, start, stop, status etc.), cobrindo todas as versões. Nota: `command_versions` segue vazio para praticamente todo comando — a sintaxe é estável entre R81.10/R81.20/R82 na fonte oficial; diferenças reais entre versões (quando existem) já são registradas via `command_diffs` por comando, não por duplicar o card inteiro.
